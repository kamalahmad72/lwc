...
const server = http.createServer((request, response) => { // Alternatively, no assignment and use chaining: http.createServer((request, response) => {}).listen();
 ...
 request.on("error", err => {
  ...
  // Three request.url are handled; 1.) / (root request), 2. /form_submitted (form submitted request), 3.) /favicon.ico (favicon request).
  ...
  if (request.url === "/form_submitted"){ // Handle form submitted request.
   ...
   response.statusCode = 200;
   response.setHeader("Content-Type", "text/html"); // 'text/plain' outputs text to web browser. 'text/html' outputs HTML to web browser.
...
<html lang='en'>
 ...
 <body>
  ...
  <form method='post' enctype='application/x-www-form-urlencoded' action='http://localhost:8001/form_submitted'><!-- Upon form submit, web browser address bar points to resource specified by action attribute. -->
   ...
   <table>
    ...
    <thead>
     <tr>
      ...
      <th>Control Data Sent On Last Submit</th>
     </tr>
    </thead>
    <tbody>
     <tr>
   ...
   <table>
    ...
    <tfoot>
     <tr>
      <td colspan='2'><a id='1_checkboxes'></a><a href='#1_checkboxes_return'>(1)</a> For checkbox, data is sent only if the checkbox is checked. For example, for a checkbox with <code>name='nameAttributeValue'</code> attribute: 1.) if the checkbox is unchecked, nothing is sent and the JavaScript <code>nameAttributeValue</code> property is <code>undefined</code>; and 2.) if the checkbox is checked, <code>nameAttributeValue=on</code> is sent if the <code>value</code> attribute is not used, or <code>nameAttributeValue=valueAttributeValue</code> is sent if the <code>value</code> attribute is used.</td>
     </tr>
    </tfoot>
   </table>
   ...
   <table>
    ...
    <tfoot>
     <tr>
      <td colspan='2'><a id='1_radioButtons'></a><a href='#1_radioButtons_return'>(1)</a> For radio buttons, data is sent only if a radio button is checked and it corresponds only to the checked radio button. For example, for radio buttons with <code>name='nameAttributeValue'</code> attribute: 1.) if the radio buttons are unchecked, nothing is sent and the JavaScript <code>nameAttributeValue</code> property is <code>undefined</code>; and 2.) if a radio button is checked, <code>nameAttributeValue=valueAttributeValue</code> is sent, where <code>valueAttributeValue</code> is from the <code>value</code> attribute of the checked radio button.</td>
     </tr>
    </tfoot>
   </table>
  ...
  if (request.url === "/favicon.ico"){ // Handle favicon request. Upon form submit, web browser performs two requests. First, web browser requests resource specified by form action attribute, which in this example is request.url === "/". Second, web browser requests favicon.ico, which is request.url === "/favicon.ico". At end of both requests the end event fires. The first request provides the form data used by the on data event code to construct the requestBody string, and, therefore, when the first end event fires, the on end event code to process the form data and generate the response proceeds without error. The second request does not provide the form data used by the on data event code to construct the requestBody string, and, therefore, when the second end event fires, the on end event code to process the form data errors on line 149: TypeError: Cannot read property 'indexOf' of undefined, which is a reference that requestBodyArray[4] does not exist. Alternatively, omit favicon request router/handler and wrap the on end event code to process the form data and generate the response in a router/handler for the first request: if (request.url === "/"){). Alternatively, use router/handler for both requests.
   response.statusCode = 200;
