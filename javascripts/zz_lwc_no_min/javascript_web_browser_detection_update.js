...
var BrowserDetectionUpdateUtil = {
 ...
 browserDetectionUpdate: function(){
  ...
  // IMPORTANT: Because FF 16+ major releases (e.g., FF58) and point releases (e.g, FF58.0.1) have identical user-agent string values, the logic for FF is slightly different than, for example, SM.
   latestVersionFFPerUaString = "64.0", // Enter latest version FF per user-agent string value, not latest actual version FF. Set to string. Eg, if latest version FF is 60.0.1 but its user-agent string reports 60.0, set to string "60.0" not "60.0.1". REQUIRED.
   ...
   latestVersionFF = "64.0.1", // Enter latest actual version FF, not latest version FF per user-agent string value. Set to string. Eg, if latest version FF is 60.0.1 but its user-agent string reports 60.0, set to string "60.0.1". REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. However, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, set to latestVersionFFPerUaString. This way, if FF is FF beta, the report will display the correct latest known official version FF.
   listVersionsFFWithIdenticalUaStringExceptLatestVersionFF = "64.0", // List versions FF with identical user-agent string values except latest version FF. Set to string. Eg, if latestVersionFF = "60.0.1", set to string "60.0" (NOTE: no trailing comma in string if list contains only a single version). Eg, if latestVersionFF = "60.0.2", set to string "60.0, 60.0.1," (NOTE: trailing comma in string if list contains multiple versions). REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. Therefore, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, can leave value as is, can set value to empty string "", or can set value to anything.
/*
