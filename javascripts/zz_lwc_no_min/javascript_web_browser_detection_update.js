...
var BrowserDetectionUpdateUtil = {
 ...
 browserDetectionUpdate: function(){
  ...
  // IMPORTANT: Because FF 16+ major releases (e.g., FF58) and point releases (e.g, FF58.0.1) have identical user-agent string values, the logic for FF is slightly different than, for example, SM.
   latestVersionFFPerUaString = "61.0", // Enter latest version FF per user-agent string value, not latest actual version FF. Set to string. Eg, if latest version FF is 50.0.1 but its user-agent string reports 50.0, set to string "50.0" not "50.0.1". REQUIRED.
   isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, // Is there FF point releases with identical user-agent string value as FF major release? Set to boolean true or false. Eg, if FF 50.0 and FF 50.0.1 (+/- and FF 50.0.2, etc) have identical user-agent strings reporting 50.0, set to boolean true. Eg, if there is a FF 50.0 but there is no FF 50.0.1, set to boolean false. REQUIRED.
   latestVersionFF = "61.0", // Enter latest actual version FF, not latest version FF per user-agent string value. Set to string. Eg, if latest version FF is 50.0.1 but its user-agent string reports 50.0, set to string "50.0.1". REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. However, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, set to latestVersionFFPerUaString. This way, if FF is FF beta, the report will display the correct latest known official version FF.
   listVersionsFFWithIdenticalUaStringExceptLatestVersionFF = "61.0", // List versions FF with identical user-agent string values except latest version FF. Set to string. Eg, if latestVersionFF = "50.0.1", set to string "50.0" (NOTE: no trailing comma in string if list contains only a single version). Eg, if latestVersionFF = "50.0.2", set to string "50.0, 50.0.1," (NOTE: trailing comma in string if list contains multiple versions). REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. Therefore, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, can leave value as is, can set value to empty string "", or can set value to anything.
/*
   latestVersionSM = "2.40",
*/
   latestVersionCH = "67.0.3396.99",
   latestVersionOP = "54.0.2952.41",
   latestVersionVVPerUaString = "1.96.1147.47", // Line added 30 Jun 2017 for Vivaldi 1.10+ user-agent string value does not report correct version of Vivaldi.
   latestVersionVV = "1.15.1147.47";
