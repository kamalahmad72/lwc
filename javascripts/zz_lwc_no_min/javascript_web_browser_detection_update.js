...
var BrowserDetectionUpdateUtil = {
 ...
 browserDetectionUpdate: function(){
  ...
  // IMPORTANT: Because FF 16+ major releases (e.g., FF58) and point releases (e.g, FF58.0.1) have identical user-agent string values, the logic for FF is slightly different than, for example, SM.
   ...
   latestVersionFF = "62.0.3", // Enter latest actual version FF, not latest version FF per user-agent string value. Set to string. Eg, if latest version FF is 50.0.1 but its user-agent string reports 50.0, set to string "50.0.1". REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. However, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, set to latestVersionFFPerUaString. This way, if FF is FF beta, the report will display the correct latest known official version FF.
   listVersionsFFWithIdenticalUaStringExceptLatestVersionFF = "62.0, 62.0.2,", // List versions FF with identical user-agent string values except latest version FF. Set to string. Eg, if latestVersionFF = "50.0.1", set to string "50.0" (NOTE: no trailing comma in string if list contains only a single version). Eg, if latestVersionFF = "50.0.2", set to string "50.0, 50.0.1," (NOTE: trailing comma in string if list contains multiple versions). REQUIRED ONLY IF isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = true. Therefore, if isFFPointReleasesWithIdenticalUaStringAsFFMajorRelease = false, can leave value as is, can set value to empty string "", or can set value to anything.
/*
   latestVersionSM = "2.40",
*/
   latestVersionCH = "69.0.3497.100",
   latestVersionOP = "56.0.3051.43",
   latestVersionVV = "2.0.1309.37";
